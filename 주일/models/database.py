...
def db():
    ...
    busy timeout과 WAL 모드를 적용해 읽기/쓰기 잠금 경합 시 deadlock 없이 대기한다.
    """
    ...
    try:
        ...
        logger.exception('DB pragma 설정 실패')
    return conn
...
def init_db():
    ...
    try:
        ...
        # 중복 출석 방지용 유일 인덱스 (user_id, check_date, mode, env)
        _idx = [r[1] for r in conn.execute('PRAGMA index_list(attendance)')]
        if 'uq_attendance' not in _idx:
            conn.execute(
    ...
    try:
        ...
        logger.exception('expire_newbie_notes 실패')
